...
const Account = ({ user }) => {
	...
	const stats_by_mode = useMemo(() => {
		...
		return Object.assign(
			{},
			...['time_game', 'words_game', 'quote_game', 'gibberish_game'].map(
				(m) => {
					const last_ten = () => {
						const games = stats[m].all_games.slice(-10);
						const total_chars = games[0]
							? games.map((g) => g.total_chars).reduce(sum)
							: 0;
						const correct_chars = games[0]
							? games.map((g) => g.correct_chars).reduce(sum)
							: 0;
						const time = games[0]
							? games.map((g) => parseInt(g.time)).reduce(sum)
							: 0;
	...
	const overall_stats = useMemo(() => {
		...
		const agg = {
			...
			total_correct_chars: modes
				.map((m) => stats[m].total_correct_chars)
				.reduce(sum),
		};
			...
			<span tabIndex={0} className='outline-none'>
				{stats ? (
					<AccountGameStats
						mode={mode}
						stats={
							mode === 'all' ? overall_stats : stats_by_mode[`${mode}_game`]
						}
					/>
				) : (
					<span>
